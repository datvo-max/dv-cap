...
let currentScanMode = ''; // Biến theo dõi: 'auto' hoặc 'manual'
...
function continueScanning() {
  closeModal();
  // KIỂM TRA CHẾ ĐỘ ĐỂ CHẠY ĐÚNG HÀM
...
function startScanner() {
  currentScanMode = 'auto'; // Đánh dấu đang dùng camera tự động
  ...
  // Nếu camera đang chạy rồi thì không cần gọi lại
  ...
  html5QrCode.start(cameraConstraints, config, (decodedText) => {
    // 1. Phân tích & Lưu dữ liệu
    const personData = parseCCCD(decodedText);
    scannedData.push(personData);
    updateTable();

    // 2. Dừng luồng video ngay lập tức để tránh quét đúp cùng 1 mã liên tục
    stopScanner();

    // 3. Hiện bảng thông báo Modal
    showModal(
      "Quét tự động thành công!",
      "Đã thêm dữ liệu của: " + (personData["Họ và Tên"] || "Mã QR") + "\n\nBạn có muốn đưa thẻ khác vào quét tiếp không?",
      true
    );

  }).catch(err => {
    // Bỏ qua các lỗi quét khung hình trống ngầm định của thư viện
...
function scanImage(event) {
  currentScanMode = 'manual'; // Đánh dấu đang dùng camera gốc (thủ công)
...
function processImageFile(file, event) {
  html5QrCode.scanFile(file, false)
    .then(decodedText => {
      ...
      showModal("Xử lý ảnh thành công!", "Đã thêm dữ liệu của: " + (personData["Họ và Tên"] || "Mã QR") + "\n\nBạn có muốn chụp thẻ tiếp theo không?", true);
      event.target.value = "";
    })
    .catch(err => {
