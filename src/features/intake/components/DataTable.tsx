...
export default function DataTable({ data, onDeleteRow }: DataTableProps) {
  ...
  if (debouncedSearchTerm !== prevSearch) {
  ...
  const filteredData = useMemo(() => {
    ...
    const normalized = removeVietnameseTones(debouncedSearchTerm.toLowerCase().trim());
    return data.filter((item) => {
      const combined = [item.fullName, item.idNumber, item.address, item.fatherName, item.motherName]
        .filter(Boolean)
        .map((s) => removeVietnameseTones(String(s).toLowerCase()))
        .join(" ");
      return combined.includes(normalized);
    });
  }, [data, debouncedSearchTerm]);

  // Kiểm tra lại trang hiện tại khi dữ liệu thay đổi
  useEffect(() => {
  ...
  return (
    <div ref={tableTopRef} className="bg-white rounded-xl shadow-sm border overflow-hidden w-full flex flex-col">
      ...
      <div className="p-4 border-b border-gray-100 bg-gray-50/50">
        <div className="relative flex items-center">
          ...
          <input
            ref={searchInputRef}
            type="text"
            placeholder="Tìm kiếm theo Họ tên, Số ĐDCN, Địa chỉ..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none bg-white font-medium"
          />
          {searchTerm && (
            <button
              onClick={() => {
        ...
        {searchTerm && (
          <p className="text-[11px] text-gray-500 mt-1.5 ml-1">
            Tìm thấy <span className="font-bold text-blue-700">{totalItems}</span> kết quả trong tổng số <span className="font-bold">{data.length}</span> bản ghi
          </p>
        )}
      </div>
      ...
      <div className="overflow-x-auto min-h-100">
        <table className="w-full text-xs text-left border-collapse text-gray-600 whitespace-nowrap">
          <thead className="bg-gray-100 text-gray-700 font-bold border-b border-gray-200">
            <tr>
              <th className="px-3 py-3 text-center w-10 border-b border-gray-200">STT</th>
              <th className="px-3 py-3 border-b border-gray-200">Số ĐDCN</th>
              <th className="px-3 py-3 border-b border-gray-200">Họ và Tên</th>
              <th className="px-3 py-3 border-b border-gray-200">Ngày Sinh</th>
              <th className="px-3 py-3 border-b border-gray-200">Giới Tính</th>
              <th className="px-3 py-3 max-w-xs border-b border-gray-200">Địa Chỉ Thường Trú</th>
              <th className="px-3 py-3 border-b border-gray-200">Ngày Cấp</th>
              <th className="px-3 py-3 border-b border-gray-200">Số ĐDCN Đã Huỷ</th>
              ...
              <th className="px-3 py-3 text-center w-16 sticky right-0 bg-gray-100 shadow-[-4px_0_10px_rgba(0,0,0,0.02)] border-b border-gray-200">
                Thao tác
              </th>
            </tr>
          </thead>
              ...
              paginatedData.map((item, index) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-indigo-50/40 transition-colors">
                    <td className="px-3 py-2 text-center text-gray-400 font-normal">{actualIndex}</td>
                    ...
                    <td className="px-3 py-2 text-gray-700">{item.gender}</td>
                    <td
                      className="px-3 py-2 max-w-xs overflow-hidden text-ellipsis text-gray-500 font-normal"
                      title={item.address}
                    >
                      {item.address}
                    </td>
                    <td className="px-3 py-2 text-gray-700">{item.issueDate}</td>
                    <td className="px-3 py-2 text-gray-600">{item.canceledIdNumber || "-"}</td>
                    <td className="px-3 py-2 text-gray-600">{item.fatherName || "-"}</td>
                    <td className="px-3 py-2 text-gray-600">{item.motherName || "-"}</td>

                    <td className="px-3 py-2 text-center relative right-0 bg-white shadow-[-4px_0_10px_rgba(0,0,0,0.02)] transition-colors">
      ...
      <div className="p-3 border-t border-gray-100 bg-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4 text-[11px] text-gray-600 font-medium">
        ...
        <div>
          Đang xem{" "}
          <span className="font-bold text-gray-800">{totalItems === 0 ? 0 : startIndex + 1}</span>
          {" "}-{" "}
          <span className="font-bold text-gray-800">{Math.min(startIndex + itemsPerPage, totalItems)}</span>
          {" "}trong tổng số{" "}
          <span className="font-bold text-blue-700">{totalItems}</span> thẻ
        </div>
