...
export function useScannerApp() {
  ...
  const startWebcam = async () => {
    ...
    // Chờ DOM render phần tử #reader — polling thay vì timeout cố định
    ...
    const elementReady = await waitForElement('reader');
    if (!elementReady) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Kiểm tra phần tử DOM tồn tại trước khi khởi tạo Html5Qrcode
    const fileScannerEl = document.getElementById('file-scanner');
    if (!fileScannerEl) {
      showToast("❌ Lỗi hệ thống: Không tìm thấy phần tử xử lý ảnh.", "error");
      e.target.value = "";
      return;
    }
    ...
    try {
      ...
      console.error("Lỗi khởi tạo Html5Qrcode cho file:", initError);
      showToast("❌ Không thể khởi tạo bộ đọc QR. Hãy thử lại.", "error");
      e.target.value = "";
      return;
    }
    ...
    try {
      ...
      // Nếu không có kết quả nào cả (tất cả fail nhưng failCount = 0 — trường hợp lạ)
      if (newRecords.length === 0 && duplicateNames.length === 0 && failCount === 0) {
        showToast("⚠️ Không phát hiện mã QR nào trong ảnh.", "warning");
      }
    } catch (outerError) {
      console.error("Lỗi xử lý file upload:", outerError);
      showToast("❌ Có lỗi xảy ra khi xử lý ảnh. Vui lòng thử lại!", "error");
    }
